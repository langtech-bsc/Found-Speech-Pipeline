"""
Additional segment hypothesis enrichment utilities.
"""
...
def normalize_text_for_enrichment(text: str, lang: str) -> str:
    try:
        from fsp.core.text import clean_text as core_clean_text

        return core_clean_text(text, lang, False, False)
    except Exception:
        from scripts.clean_and_expand import clean_text as legacy_clean_text

        return legacy_clean_text(text, lang, False, False)
...
def selected_specs(langs: list[str], model_names: list[str] | None) -> list[ModelSpec]:
    ...
    if model_names:
        ...
        if missing:
            raise ValueError(f"Unknown enrichment models: {', '.join(sorted(missing))}")
...
def transcribe_phi4_multimodal_audio(model: dict[str, Any], audio_path: Path, _lang: str) -> str:
    audio, sample_rate = librosa.load(str(audio_path), sr=16000, mono=True)
    processor = model["processor"]
    user_msg = {
        "role": "user",
